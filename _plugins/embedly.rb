...
module Jekyll
  class Embedly < Liquid::Tag
    ...
    def render(context)
      ...
      raise 'You must provide an embed.ly api key' if @config.nil?
      @api_key = @config['api_key']
      raise "You must provide embed.ly api key." if @api_key.nil?
    ...
    def compose(json_rep)
      ...
      if type == 'photo'
        ...
        html = json_rep.has_key?('html') ? json_rep['html'] : "<a href=\"#{json_rep['url']}\">#{json_rep['title']}</a>"
      end
